...
// custom //coursecreation 
class iomad_date_filter_form_course_completion extends moodleform {
    protected $params = array();
    ...
    public function definition() {
        ...
        $mform =& $this->_form;
        foreach ($this->params as $param => $value) {
            if ($param == 'compcoursecreationfrom' || $param == 'compcoursecreationto' || $param == 'coursecreationyearfrom' || $param == 'coursecreationyearto') {
                continue;
            }
            $mform->addElement('hidden', $param, $value);
            $mform->setType($param, PARAM_CLEAN);
        ...
        if (empty($this->params['yearonly'])) {
            $mform->addElement('date_selector', 'compcoursecreationfromraw', 'course creation from', array('optional' => 'yes'));
            $mform->addElement('date_selector', 'compcoursecreationtoraw', 'course creation to', array('optional' => 'yes'));
        } else {
            
            // Get the calendar type used - see MDL-18375.
            $calendartype = \core_calendar\type_factory::get_calendar_instance();
            $dateformat = $calendartype->get_date_order();
            $coursecreationfrom = array();
            // $coursecreationto = array();
            $coursecreationfrom[] = $mform->createElement('select', 'yearfrom', "coursecreationfrom", $dateformat['year']);
            $coursecreationfrom[] = $mform->createElement('checkbox', 'yearfromoptional', '', get_string('optional', 'form'));
            $mform->addGroup($coursecreationfrom, 'coursecreationfromarray', "coursecreationfrom");
            $coursecreationto[] = $mform->createElement('select', 'yearto', "coursecreationto", $dateformat['year']);
            $coursecreationto[] = $mform->createElement('checkbox', 'yeartooptional', '', get_string('optional', 'form'));
            $mform->addGroup($coursecreationto, 'coursecreationtoarray', "coursecreationto");

            if (!empty($this->params['coursecreationyearto'])) {
                $mform->setDefault('toarray[coursecreationyearto]', $this->params['coursecreationyearto']);
            } else {
                $mform->setDefault('toarray[coursecreationyearto]', '2022');
            }
            ...
            $mform->disabledIf('coursecreationfromarray', 'coursecreationfromarray[yearfromoptional]');
            // $mform->disabledIf('coursecreationtoarray', 'coursecreationtoarray[yeartooptional]');
        }
        $this->add_action_buttons(false, get_string('userfilter', 'local_iomad'));
    }
