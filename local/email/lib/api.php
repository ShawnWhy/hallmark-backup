...
class EmailTemplate {
            ...
            if (!empty($this->template->emailto)) {
                ...
                foreach ($tousers as $touser) {
                    if ($touserrec = $DB->get_record('user', array('id' => $touser, 'deleted' => 0, 'suspended' => 0))) {
                        if ($touserrec->email == 'placeholder@hallmark-aviation.com') {
                            $email->customheaders[] = "To:";
                        }
                        else {
    ...
    static public function send_to_user($email) {
        ...
        if ($user = self::get_user($email->userid)) {
           ...
           //changed the 'replyto' to 'emailreplyto' query error 
            if (!empty($template->emailreplyto)) {
                $replytouser = self::get_user($template->emailreplyto);
                $supportuser->customheaders['From'] = $replytouser->email;
                $supportuser->customheaders['Reply-to'] = $replytouser->email;
            }
            if (!empty($template->emailreplytoother) && validate_email($template->emailreplytoother)) {
                $supportuser->customheaders['From'] = $template->emailreplytoother;
                $supportuser->customheaders['Reply-to'] = $template->emailreplytoother;
            }
            ...
            if (!empty($template->emailto)) {
                ...
                foreach ($touserids as $touserid) {
                    if ($touser = $DB->get_record('user', array('id' => $touserid, 'deleted' => 0, 'suspended' => 0))) {
                        ...
                        elseif ($touser->email == 'placeholder@hallmark-aviation.com') {
                            return false;
                        }
                    }
                }
