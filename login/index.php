...
// Custom Code Start
// Deal with any passed company parameters so we can set the theme and other good stuff.
...
if (!empty($wantedcompanyid) && !$company = $DB->get_record('company', array('id'=> $wantedcompanyid, 'shortname'=>$wantedcompanyshort))) {
    ...
    // Custom Code Start
    error_log(print_r($company->theme,true));
    error_log(print_r($_SERVER["SERVER_NAME"], true));
    // Custom Code End
...
// Custom Code Start
// Set the theme if the server hostname matches one of ours.
...
// Custom Code Start
// Changes to display the instructions.
