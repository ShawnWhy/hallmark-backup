...
class block_iomad_reports extends block_base {
    ...
    public function get_content() {
        ...
        foreach ($reports as $report) {
            if (iomad::has_capability("local/$report:view", $this->context)) {
                ...
                if ((empty($USER->theme) && (strpos($CFG->theme, 'iomad') !== false)) || (strpos($USER->theme, 'iomad') !== false)) {
                    ...
                    $this->content->text .= '<div class="iomadicon test">' . $icon ;
                }
        ...
        // Custom Code Start
        $learning_path_all = '<a href="/blocks/configurable_reports/viewreport.php?id=6"><div class="iomadlink mr-2"><div class="iomadicon report"> <div class="fa fa-topic fa-clock-o"><div class="fa fa-action fa-bar-chart-o"></div></div></div><div class="actiondescription">Learning path completion report - all</div></div></a>';
        $this->content->text .=$learning_path_all;
        // Custom Code End
        $this->content->text .= '</div>';
